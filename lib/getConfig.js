...
const getConfig = () => {
  const tasksConfigPathJS = path.resolve(process.cwd(), '.tasksrc.js');
  const tasksConfigPathCJS = path.resolve(process.cwd(), '.tasksrc.cjs');
  ...
  if (!fs.existsSync(tasksConfigPathJS) && !fs.existsSync(tasksConfigPathCJS)) {
    if ('tasks' in pkg) {
      return {
        scripts: pkg.scripts || {},
        tasks: pkg.tasks,
  ...
  const tasksConfigPath = fs.existsSync(tasksConfigPathJS) ? tasksConfigPathJS : tasksConfigPathCJS;
