...
const getConfig = () => {
  ...
  if (!fs.existsSync(tasksConfigPath)) {
    if ('tasks' in pkg) {
      ...
      throw new Error(chalk`Can't find either a {yellow .tasksrc.js} in your project root or tasks config in {yellow package.json}.`);
    }
  }
  const tasksConfig = require(tasksConfigPath);

  if (!('scripts' in tasksConfig)) {
    tasksConfig.scripts = pkg.scripts || {};
  }
