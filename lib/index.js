...
export default class Runner {
  ...
  async runTask(task, overrideArgs = [], overrideKwargs = {}) {
    ...
    const spawnScript = (script, scriptArgs) => {
      if (this.verbose) {
    ...
    for (const i in run) {
      ...
      // If not in scripts, run with unconstrued args and continue
      if (!(cmd in scripts)) {
        spawnScript(cmd, unconstruedArgs);
        continue;
      }

      const parseScript = (scriptString) => {
        const prefixedScript = /^npx /i.test(scriptString) ?
          scriptString : `npx ${scriptString}`;
        const script = parser(prefixedScript)._[0];
        const scriptPosArgs = unparser(
        ...
        spawnScript(script, scriptArgs);
      };

      // Complex scripts, e.g., cmd1 && cmd2
      if (/&&/.test(scripts[cmd])) {
        const splitScripts = scripts[cmd].split('&&').map(s => s.trim());
        for (const splitScript of splitScripts) parseScript(splitScript);
      } else {
        parseScript(scripts[cmd]);
      }
    }
  }
}
