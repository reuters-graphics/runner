...
export default class Runner {
  constructor(testConfig = null, testArgs = null) {
    ...
    this.verbose = Boolean(argv.verbose);
    ...
    if (!this.task) throw new Error(chalk`{red runner:} {gray Pass a task name to runner like:} {yellow $ runner <task>}`);
  ...
  runTask(task, overrideArgs = [], overrideKwargs = {}) {
    ...
    for (const i in run) {
      ...
      if (this.verbose) {
        console.log(chalk`{red.dim runner:} {gray.dim ${script} ${scriptArgs.join(' ')}}`);
      }
